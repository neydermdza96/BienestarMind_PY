...
+ Validacion: no se permiten fechas pasadas (servidor + cliente)
"""
...
class ReservaEspacioForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['fecha_reserva', 'hora_inicio', 'duracion', 'espacio', 'ficha', 'motivo_reserva']
        widgets = {
    ...
    def clean_fecha_reserva(self):
        ...
        if fecha and fecha < hoy:
            raise forms.ValidationError(
                ...
                f'Fecha minima: {hoy.strftime("%d/%m/%Y")}. '
...
class ReservaElementoForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['fecha_reserva', 'elemento', 'ficha', 'descripcion_reserva']
        widgets = {
            'fecha_reserva': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
...
def lista_espacios(request):
    ...
    return render(request, 'reservas/lista_espacios.html', {
        'reservas': qs, 'estados': ReservaEspacio.ESTADO_CHOICES,
        'sedes': Sede.objects.all(),
        'q': q, 'estado': estado, 'sede': sede,
...
def crear_reserva_elemento(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        try:
            ...
            messages.success(request, 'Elemento reservado. Notificaciones enviadas.')
        except Exception:
